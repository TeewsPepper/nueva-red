...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const canCreateActividad = (): boolean => hasRole(['pastor', 'lider', 'miembro'])
  const canJoinActividad = (): boolean => hasRole(['pastor', 'lider', 'miembro', 'visitante'])
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        console.error('Error al verificar token:', apiError.response?.data?.message)
        localStorage.removeItem('token')
        localStorage.removeItem('user')
        delete api.defaults.headers.common['Authorization']
      } finally {
  ...
  const logout = async () => {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Error en servidor, limpiando estado local:', serverError)
      }
